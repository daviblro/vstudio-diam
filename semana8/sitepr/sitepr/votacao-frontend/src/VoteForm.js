...
function VoteForm({ options, question, toggle }) {
  ...
  const voteAndCloseModal = (event) => {
    ...
    if (selectedOption >= 0) options[selectedOption].votos++;
    axios
      .put(URL_OPTION + options[selectedOption].pk, options[selectedOption])
      .then();
    /*
 ou então:
                      ...
                      <FormGroup check>
                        <Label>
                          <input
                            ...
                            value={index++}
                            className="form-check-input"// Onde esta definida, prov lado nenhum
